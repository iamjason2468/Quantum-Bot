...
async def monitor_position(account_id, trade2_id, trade3_id, entry_price, sl_price, tp1_price, symbol, direction):
    """
    Monitors position and moves SL to break even after TP1 is hit.
    """
    api = MetaApi(TOKEN)
    connection = None
                ...
                if not be_activated:
                    if (direction == "BUY" and current_price >= tp1_price) or \
                       (direction == "SELL" and current_price <= tp1_price):
                        ...
                        # Modify SL for remaining trades to entry price (Break Even)
                        try:
                            # Modify Trade 2 SL
                        ...
                        be_activated = True
                        logger.info(f"✅ Break Even activated! Monitoring complete.")
                        break  # Exit monitoring after BE is set
...
async def place_scaled_trades(account_id, action, symbol, volume, entry, sl, tp1, tp2, tp3, use_adaptive=True):
    ...
    try:
        ...
        # Place Trade 1 (TP1 only)
        if action.lower() == "buy":
            trade1_result = await connection.create_market_buy_order(
                symbol, trade1_size, stop_loss=sl, take_profit=tp1, comment="TP1"
            )
        else:
        ...
        # Place Trade 2 (TP2 only)
        if action.lower() == "buy":
            trade2_result = await connection.create_market_buy_order(
                symbol, trade2_size, stop_loss=sl, take_profit=tp2, comment="TP2"
            )
        else:
        ...
        # Place Trade 3 (TP3 only)
        if action.lower() == "buy":
            trade3_result = await connection.create_market_buy_order(
                symbol, trade3_size, stop_loss=sl, take_profit=tp3, comment="TP3"
            )
        else:
        ...
        # Start background monitoring for Break Even
...
def webhook():
    try:
        ...
        # NEW: Get TP/SL levels from webhook
